...
Intuitively, this means that knowing if $A$ happened tells us nothing about whether $B$ happened. This can also be written as
\[
P(A | B) = P(A),
\]
and 
...
Bootstrap sampling can be implemented by sampling with replacement.
...
Hypotheses about the expected salary relate to [[*Conditional expectationmhe:][conditional expectation]]
