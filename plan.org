...
*** The murder problem
Deal $n$ cards, one for each student.

1. Throw dice to randomly select the killer: note the number down.

2. Throw dice to randomly select a suspect. Call the suspect to the
   board. He is now on trial: somebody saw somebody matching their
   description and he was found in the neighbourghood. There is no
   other evidence.
...
3. Perform a DNA test on the suspect, revealing only the result.
   (a) If it's 0 then the test is positive.
   (b) If they are the killer, then the test is positive.
   (c) Otherwise, it is negative.

4. Tell the students that the probability that the DNA test is
   positive if the person is innocent is 10%. Ask them what is their
   belief *now* that the suspect is guilty?

   Take a vote again to do so.

5. Now consider the case where everybody is a suspect. How many are false positives?


6. Explanation

   We can describe this in terms of the conditional probability.
   Let $D$ denote a positive test and $\neg D$ a negative one. Then
   \[
   P(D | H_0) = 0.1 \tag{false positive rate}
   \]
   
   The converse probability is 
   \[
   P(H_0 | D) = P(D \cap H_0) / P(D) = P(D | H_0) P(H_0) / P(D)
   \]

   To calculate $P(D)$ we need to use marginalisation:
   \[
...
*** Instructions
 ...
 following questions:
>>>>>>> 0d76e5d06b54711dec940d6324fa6d514d577b18
