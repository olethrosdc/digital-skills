...
The random variable is the party affiliation $f : \Omega \to \{R, G, B\}$ with
\[
f(\omega) = 
\begin{cases}
R, & \omega \in \{0,1\}\\
...
all subsets $S \subsets \Omega$.  What is then the probability that
somebody supports the Red party?

The probability that $f = R$, which we write informally as $\Pr(f = R)$,
 is simply the probability of all $\omega$ that lead to $R$, that is:
\[
\Pr(f = R) = P(\{\omega : f(\omega) = R\}) = P(\{0,1\}) = 2.
\]
