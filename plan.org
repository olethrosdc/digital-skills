...
*** Explanation
 ...
 - Assuming $P(D | H_1) = 1$, and setting $P(H_0) = q$, this gives
	...
	\]
*** Python example
	Let us assume $P(D | H_0) = 10^{-6}$.
#+BEGIN_SRC python
	import numpy as np
	base_rate = 0.000001
	population_size = 10000000
	n_positive_tests = np.random.binomial(population_size, base_rate)
	return n_positive_tests
#+END_SRC
...
This is an instance of a multiple-hypothesis test.
	
*** Confidence intervals for binary variables

This is an instance of a credible interval test.
...
*** Types of hypothesis testing
	There are many types of hypotheses we can do
#+BEAMER \pause
**** Null hypothesis testing: no specific alternatives
	 - $H_0$: Are men paid the same as women?
**** Simple hypothesis testing: mutually exclusive alternatives
	 - Is the defendant innocent ($H_0$) or guilty ($H_1$)?
**** Confidence intervals: a range of alternatives
	 - What is a likely vote percentage range for political party $A$?
