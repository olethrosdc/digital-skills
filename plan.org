...
scientific hypotheses, design experiments, collect and analyse
data. You are expected to supplement this course with readings from
other courses, but no prior knowledge is assumed. Formal concepts will
...
Whenever we have a specific coin to throw in the air, there are two
things we do not know. The first is which side the coin will land
on. The second is versus uncertainty about the coin bias: is the
probability of landing heads exactly 50%? How can we quantify this?
Discuss in class.
